...
constructor(
    ...
    override fun start() {
        resolver.registerContentObserver(
            LineageSettings.Secure.getUriFor(LineageSettings.Secure.QS_UI_STYLE),
            false,
            styleObserver,
            UserHandle.USER_ALL,
        )
    ...
    private fun ensureCurrentStyleInitialized(userId: Int) {
        backgroundExecutor.execute {
            ...
            val saved = LineageSettings.Secure.getStringForUser(resolver, key, userId)
            if (saved == null) {
                val active =
                ...
                val initialized =
                    if (applied == QSStyleRuntime.STYLE_A11) {
                        addDefaultA11Items(sanitizeTiles(active, allowA11Items = false))
                    } else {
                ...
                LineageSettings.Secure.putStringForUser(resolver, key, initialized, userId)
            } else if (applied == QSStyleRuntime.STYLE_A11) {
                val migrated = migrateA11AutoBrightness(saved)
                if (migrated != saved && setActiveTiles(userId, migrated)) {
                    LineageSettings.Secure.putStringForUser(resolver, key, migrated, userId)
                }
            }
    ...
    private fun handleRequestedStyle(userId: Int) {
        ...
        val requested =
            LineageSettings.Secure.getIntForUser(
                resolver,
                LineageSettings.Secure.QS_UI_STYLE,
    ...
    private fun switchStyle(userId: Int, from: Int, to: Int): Boolean {
        ...
        val oldTiles =
            sanitizeTiles(backup, allowA11Items = from == QSStyleRuntime.STYLE_A11)
        val targetKey = tilesKey(to)
        var target = LineageSettings.Secure.getStringForUser(resolver, targetKey, userId)
        if (target == null) {
            target =
        ...
        try {
            check(
                LineageSettings.Secure.putStringForUser(
                    resolver,
                    LineageSettings.Secure.QS_UI_STYLE_SWITCH_STATE,
                    state,
                    userId,
                )
            )
            ...
            check(
                LineageSettings.Secure.getStringForUser(resolver, tilesKey(from), userId) ==
                    oldTiles
            )
            check(
                LineageSettings.Secure.putStringForUser(resolver, targetKey, target, userId)
            )
            check(setActiveTiles(userId, target))
            check(
                LineageSettings.Secure.putIntForUser(
                    resolver,
                    LineageSettings.Secure.QS_UI_STYLE_APPLIED,
                    to,
                    userId,
                )
            )
    ...
    private fun addDefaultA11Items(raw: String): String {
        ...
        if (!specs.contains(AUTO_BRIGHTNESS_SPEC) && shouldInsertAutoBrightness()) {
            specs.add(insertion + 2, AUTO_BRIGHTNESS_SPEC)
        }
        return specs.joinToString(",")
    }

    private fun migrateA11AutoBrightness(raw: String): String {
        if (!shouldInsertAutoBrightness()) return raw
        val specs = raw.split(',').filter(String::isNotBlank).toMutableList()
    ...
    private fun shouldInsertAutoBrightness(): Boolean {
        return try {
            val raw =
                LineageSettings.Secure.getString(
                    resolver,
                    LineageSettings.Secure.QS_TILE_LAYOUT_A11,
                )
    ...
    private fun requestRestart(reason: String) {
        if (restartRequested.compareAndSet(false, true)) {
            restarter.restartSystemUI(reason)
        }
    }

    private fun tilesKey(style: Int): String =
        if (style == QSStyleRuntime.STYLE_A16) {
            LineageSettings.Secure.QS_TILES_A16
        } else {
            LineageSettings.Secure.QS_TILES_A11
        }
