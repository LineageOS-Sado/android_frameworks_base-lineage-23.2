...
final class MomentController {
    ...
    private void markNoAnimationLocked(Task task) {
        task.forAllActivities(activity -> {
            ...
            // A nav-handle double tap can arrive while Quickstep is collecting a transient
            // recents transition. In that case the activity is not necessarily a direct
            // participant, and Transition#setNoAnimation rejects non-participants. The intent
            // flag still suppresses the activity animation; only annotate an active Shell
            // transition when this exact activity belongs to it.
            if (activity.mTransitionController.isCollecting(activity)) {
